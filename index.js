...
const BotName = 'Woodybot'; 
const instagram = 'https://www.instagram.com/euherbson.ofc
const aktif = '08:00 - 22:00';
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:1.0\n' 
            + 'FN:Herbson\n' // Nama kamu
            + 'ORG:Araújo\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5583986694715:+55 83986694715\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!',
blocked = []
limitawal = '999999999'
		...
		case 'iri':
			...
			break
				...
				case 'thunder':
					...
					break
                case 'stiltext':
                      if (args.length < 1) return reply('sem texto pra fazer...')
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
		...
		case 'happymod':
			...
			break
				...
				case 'naruto':
					...
					client.sendMessage(from, nye, image, { caption: '!tinha várias onde eu peguei, mais escolhi essa... espeero que goste :)!', quoted: mek })
					await limitAdd(sender)
					break 
				case 'nsfwboquete':
		   ...
		   await limitAdd(sender)
			...
			case 'instaimg':
				...
				break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('*MODO AFK ATIVADO*?')                                  
                                        var nom = mek.participant
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *MÉDIO AFK ${tels} NÃO PERTURBE*`,
...
break
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('anti-link desativado com sucesso neste grupo ✔️')
					} else {
						reply('1 para ativar, 0 para desativar ')
					}
					break
				...
				case 'wibu':
                   ...
                   break
					...
					break
				case 'desligar':
				if (!isOwner) return reply('Comando so para o meu dono, quem é vc?')
...
// recursos anime, random @dark domina vida
          ...
          reply('Espere..')
				...
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 80
					? reply('vou falar não, a maior parte do texto é merda')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com meu dono...!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT Herbson ✨* 」\n*│+ wa.me/5583986694715*╰──────*「 *WOODY* 」*────*\n\n*_SE QUER SER ADMIN DO BOT Woody_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ai sim meu chapa...'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ai sim meu chapa...'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ai sim meu chapa...'})
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hentaizin ne pae...'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
			...
			reply(mess.wait)
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('Oxi, quem é tu?, tu não é meu dono...')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('Pronto chefinho lindo, ja exclui :)')
					break
				case 'bc':
					if (!isOwner) return reply('Oxi, quem é tu? tu não é meu dono...')
        ...
        case 'adm':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ok... @${mentioned[0].split('@')[0]} agora é admin do grupo!`, mentioned, true)
				...
				case 'comercaneco':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ok... @${mentioned[0].split('@')[0]} não é mais admin do grupo!`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Digite, !add 558398669xxxx pra adicionar Número no grupo')
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar, talvez seja privado')
					}
					break
				case 'banir':
				case 'ban':
				case 'expulsar':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Carai adm, comeu o caneco do cara kkkkk brabo...'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Prontinho, adeus... @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'admins':
